    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->mergeCells('A1:O1'); // sekarang sampai O
                ...
                // Tambahkan header kolom Item di O2
                $event->sheet->setCellValue('O2', 'Item');

                // Bold header tabel
                $event->sheet->getStyle('A2:O2')->getFont()->setBold(true);

                // Freeze header
                $event->sheet->freezePane('A3');

                // Rata kanan kolom Gaji & THR (sekarang N dan O)
                $event->sheet->getStyle('N:O')
                    ->getAlignment()
                    ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);

                // Format angka dengan comma separator (bisa di SUM)
                $event->sheet->getStyle('N3:O1000')
                    ->getNumberFormat()
                    ->setFormatCode('#,##0');
            },
        ];
