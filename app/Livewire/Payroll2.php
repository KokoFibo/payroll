...
class Payroll2 extends Component
{
    use WithPagination;
    ...
    public function fetchData()
    {
        ...
        try {
            $response = Http::get('http://localhost:8080/api/payroll/calculate', [
                'year' => $this->year,
                'month' => $this->month,
            ]);
            ...
            if ($response->successful()) {
                ...
                $this->data = ['error' => 'Gagal ambil data dari API'];
            }
        } catch (\Exception $e) {
    ...
    public function bankexcel()
    {
        ...
        if ($this->selected_company != 0) {
            if ($this->selected_company == 0) {
                $payroll = Payroll_1::whereIn('status_karyawan', ['PKWT', 'PKWTT', 'Dirumahkan', 'Resigned'])
    ...
    public function showDetail($id_karyawan)
    {
        ...
        $this->data_payroll = Payroll_1::with('jamkerjaid')
            ->whereMonth('date', $this->month)
            ->whereYear('date', $this->year)
    ...
    public function mount()
    {
        ...
        if (now()->day < 5) {
            ...
            $data3 = Payroll_1::orderBy('date', 'desc')->first();
    ...
    public function bonus_potongan()
    {
        ...
        foreach ($tambahan as $d) {
            ...
            $id_payroll = Payroll_1::whereMonth('date', $this->month)
                ->whereYear('date', $this->year)
                ->where('id_karyawan', $d->user_id)
                ->first();
            if ($id_payroll != null) {
                $payroll = Payroll_1::find($id_payroll->id);
    ...
    {
        $department_ids = Payroll_1::where('company_id', $this->selected_company)
            ->whereMonth('date', $this->month)
            ->whereYear('date', $this->year)
            ->distinct()
            ->pluck('department_id');
    ...
    public function updatedSelectedPlacement()
    {
        ...
        $company_ids = Payroll_1::where('placement_id', $this->selected_placement)
            ->whereMonth('date', $this->month)
            ->whereYear('date', $this->year)
            ->distinct()
            ->pluck('company_id');
        ...
        $department_ids = Payroll_1::where('placement_id', $this->selected_placement)
            ->whereMonth('date', $this->month)
            ->whereYear('date', $this->year)
            ->distinct()
            ->pluck('department_id');
    ...
    {
        // $payrollTerakhir = Payroll_1::latest('date')->first();

        // $this->select_month = Payroll_1::select(DB::raw('MONTH(date) as month'))->whereYear('date', $this->year)
        // ->distinct()
        // ->pluck('month')
        // ->toArray();
    ...
    public function bulan()
    {
        ...
        $this->select_month = Payroll_1::select(DB::raw('MONTH(date) as month'))
            ->whereYear('date', $this->year)
            ->distinct()
            ->pluck('month')
    ...
    public function render()
    {
        ...
        $this->select_year = Payroll_1::select(DB::raw('YEAR(date) as year'))
            ->distinct()
            ->pluck('year')
            ->toArray();
        // ooo

        $months = Payroll_1::select(DB::raw('MONTH(date) as month'))
            ->whereYear('date', $this->year)
            ->distinct()
            ->orderBy('date', 'desc')
            ->pluck('month')
        ...
        $total = $this->applyCommonFilters(
            Payroll_1::whereIn('status_karyawan', $statuses)
        )->sum('total');

        $payroll = Payroll_1::whereMonth('date', $this->month)
            ->orderBy('id_karyawan', 'asc')
            ->whereYear('date', $this->year)
