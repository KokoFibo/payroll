...
from
    ...
    select
        ...
        from
            presensis
        where
            presensis.user_id = rk.user_id
            and presensis.date = rk.date
            and presensis.time > ifnull(rk.second_out, '17:00')
            and presensis.time <> rk.first_in
            and presensis.time < '20:00') as overtime_in,
        (
        select
            max(presensis.time)
        from
        ...
        where
            presensis.user_id = rk.user_id
            and presensis.date = rk.date
            and presensis.time > ifnull(rk.second_out, '19:00')
                and presensis.time > '19:00') as overtime_out
    from
        vrekappresensi rk) a;
        ");
    }
